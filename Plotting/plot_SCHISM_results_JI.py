...
def get_timestamp_from_nc(ncdata, timestep):
 ...
 salinity_base=xr.Dataset()
 for f in glob.glob('/global/scratch/users/jennaisrael/run_schism/run_16/outputs/salinity*'):
    salinity_base=xr.merge([salinity, xr.open_dataset(f).mean(dim='nSCHISM_vgrid_layers')])


#Take the time average and convert to pandas df

salinity_base_tavg=salinity_base.mean(dim='time')
sal2plot=salinity_base_tavg.to_pandas()


 #salinity_decreased=xr.Dataset()
 #for f in glob.glob('/global/scratch/users/jennaisrael/run_schism/run_17/outputs/salinity*'):
   # salinity_decreased=xr.merge([salinity, xr.open_dataset(f).mean(dim='nSCHISM_vgrid_layers')])

 #salinity_increased=xr.Dataset()
 #for f in glob.glob('/global/scratch/users/jennaisrael/run_schism/run_18/outputs/salinity*'):
  #  salinity_increased=xr.merge([salinity, xr.open_dataset(f).mean(dim='nSCHISM_vgrid_layers')])
...
make_figure(sal2plot, "salinity", 0, 40)
