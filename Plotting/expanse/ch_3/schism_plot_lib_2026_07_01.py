'''
Sienna White, May 2024, modified by Jenna with help from an LLM April 2026, another modification to make the time stamps correct from Lily Tomkovic in July 2026 to fix time stamps when reading in nc files with xarray
'''
import os 
import pandas as pd 
...
class SchismOutput():
    ...
    class Result:
        def __init__(self, schism_output, variable, filenumber=1):
            ...
            # some modifications here with help from Lily Tomkovic to fix the timestep    
            if variable == "elevation":
                #dataset = xr.open_dataset(fn).elevation
                dataset = xr.open_dataset(fn, decode_times=False).elevation
                origin = parse(dataset.time.attrs["units"].split("since", 1)[1].strip(), ignoretz=True) # removes the timezone info from the origin timestring
                dataset.time.attrs["units"] = f"seconds since {origin.isoformat()}"
                dataset = xr.decode_cf(dataset)
            elif variable == "dryflag":
                #dataset = xr.open_dataset(fn).dryFlagNode
                dataset = xr.open_dataset(fn, decode_times=False).dryFlagNode
                origin = parse(dataset.time.attrs["units"].split("since", 1)[1].strip(), ignoretz=True) # removes the timezone info from the origin timestring
                dataset.time.attrs["units"] = f"seconds since {origin.isoformat()}"
                dataset = xr.decode_cf(dataset)
            else:
                dataset = xr.open_dataset(fn, decode_times=False)
                origin = parse(dataset.time.attrs["units"].split("since", 1)[1].strip(), ignoretz=True) # removes the timezone info from the origin timestring
                dataset.time.attrs["units"] = f"seconds since {origin.isoformat()}"
                dataset = xr.decode_cf(dataset)
        ...
        def set_domain(self, domain, ax):
            ...
            elif domain=="Confluence": 
                ax.set_xlim(561916,620000)
                ax.set_ylim(4200000, 4235000)
        ...
        def plot_variable(self, schism_output, domain="SSC", time="average",
            ...
            ax.set(xticks=[])#, xticklabels=labels)
            ax.set(yticks=[])
            # ax.set_xticks(ticks, minor=False)
            # ax.set_yticks(ticks, minor=False)
            
            cbar = plt.colorbar(mapper, ax=ax, orientation="vertical", shrink=0.4)
